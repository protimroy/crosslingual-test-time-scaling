...
# CUDA_VISIBLE_DEVICES=0 python language_forcing.py --model-name simplescaling/s1.1-14B --device-number 1 --output-dir processed/14B_prefix --batch-size 2 --experiment prefix
...
def main():
    ...
    model = LLM(args.model_name, tensor_parallel_size=args.device_number, dtype="bfloat16", gpu_memory_utilization=0.90, max_model_len=27440)
    ...
    for lang_code in lang_subsets:
        ...
        if args.experiment == "wait":
            ...
            prefix = LANG_PREFIX_TOKENS.get('en', "Okay, let me try to figure this out.")
            wait_token = LANG_WAIT_TOKENS.get('en', "Wait")
            system_text = "You are Qwen, created by Alibaba Cloud. You are a helpful assistant."
        elif args.experiment == "system":
            prefix = ""
            wait_token = LANG_WAIT_TOKENS.get('en', "")
            system_text = LANG_SYSTEM_PROMPT.get('en', "You are a helpful assistant.")
        else:
            prefix = LANG_PREFIX_TOKENS.get('en', "Okay, let me try to figure this out.")
            wait_token = LANG_WAIT_TOKENS.get('en', "Wait")
            system_text = LANG_SYSTEM_PROMPT.get('en', "You are a helpful assistant.")
        ...
        result_jsonl_fp = out_dir / f"s1_infer_{args.max_tokens_thinking}.jsonl"
        result_csv_fp = out_dir / f"s1_infer_{args.max_tokens_thinking}.csv"
        ...
        start_idx = 0
        with tqdm(total=len(remaining_dataset), desc=f"Processing {lang_code}") as pbar:
